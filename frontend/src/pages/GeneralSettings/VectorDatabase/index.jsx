...
export default function GeneralVectorDatabase() {
  ...
  const handleSaveSettings = async () => {
    ...
    if (error) {
      showToast(`无法保存向量数据库设置: ${error}`, "error");
      setHasChanges(true);
    } else {
      showToast("向量数据库首选项已成功保存.", "success");
      setHasChanges(false);
    }
    setSaving(false);
    closeModal();
  ...
  const VECTOR_DBS = [
    {
      ...
      description: "100% 本地向量数据库在同一实例上运行 阿里巴啦.",
    },
    // {
    //   name: "Chroma",
    //   value: "chroma",
    ...
    //   description: "您可以自行托管或在云端托管的开源矢量数据库.",
    // },
    // {
    //   name: "Pinecone",
    //   value: "pinecone",
            ...
            <div className="flex flex-col w-full px-1 md:pl-6 md:pr-[50px] md:py-6 py-16">
              <div className="w-full flex flex-col gap-y-1 pb-6 border-white border-b-2 border-opacity-10">
                ...
                <p className="text-xs leading-[18px] font-base text-white text-opacity-60">
                  {/* 这些是AnythingLLM实例如何运行的凭据和设置。重要的是这些钥匙是最新的和正确的. */}
                </p>
              </div>
