        ...
        <div className="flex flex-col w-full px-1 md:pl-6 md:pr-[50px] md:py-6 py-16">
          <div className="w-full flex flex-col gap-y-1 pb-6 border-white border-b-2 border-opacity-10">
            <div className="items-center flex gap-x-4">
              <p className="text-lg leading-6 font-bold text-white">用户</p>
            </div>
            <p className="text-xs leading-[18px] font-base text-white text-opacity-60">
              可以查看所有的用户信息，删除用户将会删除其访问权限并且无法使用。
            </p>
          </div>
          <div className="w-full justify-end flex">
            <CTAButton onClick={openModal} className="mt-3 mr-0 -mb-6 z-10">
              <UserPlus className="h-4 w-4" weight="bold" /> 新增用户
            </CTAButton>
          </div>
          <UsersContainer />
        </div>
...
const ROLE_HINT = {
  default: ["只能在所属工作区提问和分享聊天记录", "不能修改任何配置."],
  manager: [
    "可以查看、创建和删除任何一个工作区，可以修改工作区的配置",
    "可以新增、编辑和邀请新用户",
    "无法配置秘钥、修改LLM配置、vectorDB、嵌入引擎、数据库选择、日志查看和隐私设置等",
  ],
  admin: ["最高用户权限.", "能够查看、执行所有的操作"],
};

export function RoleHintDisplay({ role }) {
  return (
    <div className="flex flex-col gap-y-1 py-1 pb-4">
      <p className="text-sm font-medium text-white">权限</p>
