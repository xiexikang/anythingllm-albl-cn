...
export default function UploadFile({ workspace, fetchKeys, setLoading }) {
  ...
  const handleSendLink = async (e) => {
    ...
    if (!response.ok) {
      showToast(`上传链接时出错： ${data.error}`, "error");
    } else {
      fetchKeys(true);
  ...
  const handleUploadSuccess = () => {
    fetchKeys(true);
    showToast("文件上传成功", "success", { clear: true });
  };

  const handleUploadError = (message) => {
    showToast(`上传文件时出错: ${message}`, "error");
  };
  ...
  return (
    <div>
        ...
        {ready === false ? (
          ...
          <div className="flex flex-col items-center justify-center">
            ...
            <div className="text-white text-opacity-60 text-xs font-medium py-1">
              支持txt,word,pdf,pptx,csv,excel等
            </div>
          </div>
        ) : (
      ...
      <form onSubmit={handleSendLink} className="flex gap-x-2">
        ...
        >
          {fetchingUrl ? "获取网站..." : "获取网站"}
        </button>
      </form>
      <div className="mt-6 text-center text-white text-opacity-80 text-xs font-medium w-[560px]">
        <p>这些文件将会保存在本地，不会发送或分享给任何人。</p>
      </div>
    </div>
  );
}
