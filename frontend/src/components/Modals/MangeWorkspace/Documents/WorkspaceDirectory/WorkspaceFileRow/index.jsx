...
const PinItemToWorkspace = memo(({ workspace, docPath, item }) => {
  ...
  const updatePinStatus = async () => {
    try {
      ...
      if (!success) {
        showToast(`失败 ${!pinned ? "pin" : "unpin"} 文档.`, "error", {
          clear: true,
        });
        return;
      }
      ...
      showToast(
        `文档 ${!pinned ? "pinned to" : "unpinned from"} 工作区`,
        "success",
        { clear: true }
      );
      setPinned(!pinned);
    } catch (error) {
      showToast(`无法固定文档. ${error.message}`, "error", {
        clear: true,
      });
      return;
    }
    ...
    >
      <PushPin
        ...
        data-tooltip-content={
          pinned ? "Un-Pin from workspace" : "固定到工作区"
        }
        size={16}
  ...
  return (
    <div>
      <ArrowUUpLeft
        ...
        data-tooltip-content="将文档从工作区删除"
        onClick={onClick}
